  ...
  const observer = new IntersectionObserver((entries, observer) => {
      ...
      entries.forEach(entry => {
          ...
          if (entry.isIntersecting) {
              // Select the progress fill element within the skill item
              const progressFill = entry.target.querySelector(".skill-progress-fill");
              // Get the skill percentage from the data attribute
              const skillPercentage = entry.target.querySelector("data").value;
              // Set the width of the progress fill based on the skill percentage
              progressFill.style.width = skillPercentage + "%";
              // Stop observing the current entry (element)
...
document.addEventListener('mousemove', e => {
    const mouseX = e.pageX - 30; // Adjust the position to center the circle
